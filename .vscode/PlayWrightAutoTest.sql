...
CREATE TABLE dbo.TestCases (
  ...
  CONSTRAINT UQ_TestCases UNIQUE (CategoryId, Name),
  CONSTRAINT FK_TestCases_Suite    FOREIGN KEY (SuiteId)    REFERENCES dbo.Suites(Id) ON DELETE SET NULL,
...
/* 2) 触发器：每个 CREATE TRIGGER 必须独立批次 */
GO
...
DECLARE 
  ...
  @c1 BIGINT, @c1s1 BIGINT, @c1s3 BIGINT, @c1ds1 BIGINT, @c1ds2 BIGINT, @c1ds3 BIGINT,
  @k1 BIGINT, @k1s1 BIGINT, @k1s2 BIGINT,
  @k2 BIGINT, @k2s1 BIGINT, @k2s2 BIGINT,
  @k3 BIGINT, @k3s1 BIGINT, @k3s2 BIGINT, @k3s3 BIGINT;
...
-- DataDriven: Case1
INSERT INTO dbo.TestCases(SuiteId, CategoryId, Name) VALUES (@suiteId, @catDD, N'Case1');
...
(@c1ds1,N'prodCode',N'pp1',N'string'),
(@c1ds1,N'prodName',N'ppName1',N'string'),
(@c1ds2,N'name',N'bb',N'string'),
(@c1ds2,N'pwd',N'bbPwd',N'string'),
(@c1ds2,N'prodCode',N'pp2',N'string'),
(@c1ds2,N'prodName',N'ppName2',N'string'),
(@c1ds3,N'name',N'cc',N'string'),
(@c1ds3,N'pwd',N'ccPwd',N'string'),
(@c1ds3,N'prodCode',N'pp3',N'string'),
(@c1ds3,N'prodName',N'ppName3',N'string');

-- KeywordDriven: Case1
INSERT INTO dbo.TestCases(SuiteId, CategoryId, Name) VALUES (@suiteId, @catKD, N'Case1');
SET @k1 = SCOPE_IDENTITY();
...
SELECT @k1s1=Id FROM dbo.TestSteps WHERE CaseId=@k1 AND Ordinal=1;
SELECT @k1s2=Id FROM dbo.TestSteps WHERE CaseId=@k1 AND Ordinal=2;
...
(@k1,@k1s1,NULL,N'name',N'aa',N'string'),
(@k1,@k1s1,NULL,N'pwd',N'aaPwd',N'string'),
(@k1,@k1s2,NULL,N'menuNum',N'5',N'number');

-- KeywordDriven: Case2
INSERT INTO dbo.TestCases(SuiteId, CategoryId, Name) VALUES (@suiteId, @catKD, N'Case2');
...
(@k2,@k2s2,NULL,N'blance',N'100',N'number');

-- KeywordDriven: Case3
INSERT INTO dbo.TestCases(SuiteId, CategoryId, Name) VALUES (@suiteId, @catKD, N'Case3');
SET @k3 = SCOPE_IDENTITY();

INSERT INTO dbo.TestSteps(CaseId, Ordinal, StepName) VALUES
(@k3,1,N'Login'),
(@k3,2,N'searchProduct'),
(@k3,3,N'checkProductInfor'),
(@k3,4,N'Logout');
...
(@k3,@k3s2,NULL,N'prodCode',N'pp3',N'string'),
(@k3,@k3s3,NULL,N'prodName',N'ppName3',N'string'),
(@k3,@k3s3,NULL,N'prodSize',N'12',N'number'),
(@k3,@k3s3,NULL,N'prodPrice',N'999',N'number');
GO

/* 5) 输出过程（STRING_AGG 版） */
CREATE PROCEDURE dbo.sp_GetCasesDesignJson
  @suiteName     NVARCHAR(200) = NULL,
...
BEGIN
  ...
  ;WITH CaseScope AS (
    SELECT c.Id, c.Name, s.Name AS SuiteName, tc.Name AS CategoryName
    FROM dbo.TestCases c
    LEFT JOIN dbo.Suites s ON s.Id=c.SuiteId
    JOIN dbo.TestCategories tc ON tc.Id=c.CategoryId
    WHERE (@suiteName    IS NULL OR s.Name=@suiteName)
      AND (@categoryName IS NULL OR tc.Name=@categoryName)
      AND (@caseName     IS NULL OR c.Name=@caseName)
      AND c.IsActive=1
  )
  SELECT (
    SELECT
      cs.CategoryName AS [category],
      cs.SuiteName    AS [suiteName],
      cs.Name         AS [caseName],
      (
        SELECT
          ds.Ordinal AS [loopIndex],
          ds.Id      AS [dataSetId],
              ...
              JSON_QUERY((
                SELECT
                  CASE WHEN COUNT(*)=0 THEN '{}'
                       ELSE '{' + STRING_AGG(
                         '"' + REPLACE(src.ParamKey, '"','\"') + '":' +
                         dbo.fn_FormatJsonValue(src.ParamValue, src.ValueType)
                       , ',') + '}'
                  END
                FROM (
                  -- DataDriven
                  SELECT d.ParamKey, ddv.ParamValue, ddv.ValueType
                  FROM dbo.TestSteps st2
                  JOIN dbo.TestCases c2 ON c2.Id=st2.CaseId
                  JOIN dbo.TestCategories cat ON cat.Id=c2.CategoryId AND cat.Name='DataDriven'
                  JOIN dbo.StepParamDefs d ON d.StepId=st2.Id
                  LEFT JOIN dbo.DataDrivenValues ddv ON ddv.DataSetId=ds.Id AND ddv.ParamKey=d.ParamKey
                  WHERE st2.Id=st.Id
                  UNION ALL
                  -- KeywordDriven
                  SELECT ksv.ParamKey, ksv.ParamValue, ksv.ValueType
                  FROM dbo.KeywordStepValues ksv
                  JOIN dbo.TestCases c3 ON c3.Id=ksv.CaseId
                  JOIN dbo.TestCategories cat2 ON cat2.Id=c3.CategoryId AND cat2.Name='KeywordDriven'
                  WHERE ksv.StepId=st.Id
                    AND ksv.CaseId=c.Id
                    AND (
                         ksv.DataSetId = ds.Id
                         OR (ksv.DataSetId IS NULL AND NOT EXISTS (
                               SELECT 1 FROM dbo.KeywordStepValues k2
                               WHERE k2.CaseId=ksv.CaseId AND k2.StepId=ksv.StepId AND k2.DataSetId=ds.Id
                             ))
                        )
                ) src
              )) AS [params]
            FROM dbo.TestSteps st
            WHERE st.CaseId = c.Id
            ORDER BY st.Ordinal
            FOR JSON PATH
          ) AS [steps]
        FROM (
          SELECT ds.Id, ds.CaseId, ds.Name, ds.Ordinal
          FROM dbo.DataSets ds
          WHERE ds.CaseId=c.Id
          UNION ALL
          SELECT NULL AS Id, c.Id AS CaseId, N'Default' AS Name, 1 AS Ordinal
          WHERE EXISTS (SELECT 1 FROM dbo.TestCategories t WHERE t.Name='KeywordDriven' AND t.Id=c.CategoryId)
            AND NOT EXISTS (SELECT 1 FROM dbo.DataSets d2 WHERE d2.CaseId=c.Id)
        ) ds
        ORDER BY ds.Ordinal
        FOR JSON PATH
      ) AS [loops]
    FROM CaseScope cs
    JOIN dbo.TestCases c ON c.Id=cs.Id
    FOR JSON PATH
  ) AS CasesJson;
END
GO
...
/* 6) 验证 */
EXEC dbo.sp_GetCasesDesignJson @suiteName=N'Demo';
EXEC dbo.sp_GetCasesDesignJson @suiteName=N'Demo', @categoryName=N'DataDriven';
EXEC dbo.sp_GetCasesDesignJson @suiteName=N'Demo', @categoryName=N'KeywordDriven';
EXEC dbo.sp_GetCasesDesignJson @suiteName=N'Demo', @caseName=N'Case3';